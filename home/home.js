...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const petsAdocao = document.getElementById("pets-adocao-grid");
    const petsPerdidos = document.getElementById("pets-perdidos-grid");
    
    carregarPetsParaAdocao(petsAdocao); 
    carregarPetsPerdidos(petsPerdidos); 

    async function carregarPetsParaAdocao(gridElement) { 
        if (!gridElement) {
            console.error("Erro: O elemento com ID 'pets-adocao-grid' não foi encontrado.");
            return;
        }
    ...
    async function carregarPetsPerdidos(gridElement) { 
        if (!gridElement) {
            console.error("Erro: O elemento com ID 'pets-perdidos-grid' não foi encontrado.");
            return;
        }
        ...
        try {
            ...
            pets = pets.filter(pet => pet.categoria === "Perdido");
            pets = pets.slice(0, 3);
